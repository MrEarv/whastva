...
const ActivarCalentadorContent: React.FC<ActivarCalentadorContentProps> = ({ isWarmerActive, setIsWarmerActive, fetchMyWarmer }) => {
    ...
    const getInstances = async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.error("Error al cargar las instancias");
                setInstances([]);
            }
        } catch (error) {
    ...
    try {
        ...
        if (!response.ok || !result.estatus) {
            console.log("No se pudo obtener warmer:", result.mensaje);
            return;
        }

        console.log("Warmer obtenido correctamente:", result.data);

        return result.data;

    } catch (error) {
        console.log("Algo salió mal al obtener el warmer:", error);
    }
};
...
const handleAddMessage = async () => {
    ...
    try {
        const payload = {
            ...
            accion: "Agregar mensaje al warmer",
            data: { msg: textToSave }
        };
        ...
        if (!response.ok || !result.estatus) {
            throw new Error(result.mensaje || "No se pudo guardar el mensaje");
        }

        console.log("Mensaje guardado:", result.mensaje);
